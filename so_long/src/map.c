...
/*   Updated: 2025/07/29 00:51:32 by okhan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (j < game->map.cols)
		{
				...
				(j == 0 || j == game->map.cols - 1))
			{
				if (game->map.lines[i][j] != '1')
					ft_error_exit("Map must be surrounded by walls");
			}
			j++;
		}
		i++;
...
static void	count_elements(t_game *game, char c, int i, int j)
{
	if (c == 'P')
	{
		...
		ft_error_exit("Invalid character in map");
}
...
static void	check_characters(t_game *game)
{
	...
	if (game->map.p != 1)
		ft_error_exit("Map must contain exactly one player (P)");
	if (game->map.e != 1)
		ft_error_exit("Map must contain exactly one exit (E)");
	if (game->map.c < 1)
		ft_error_exit("Map must contain at least one collectible (C)");
}
...
void	validate_map(t_game *game)
{
	...
	if (game->map.rows < 3 || game->map.cols < 3)
		ft_error_exit("Map must be at least 3x3");
	check_walls(game);
